# AUTOGENERATED! DO NOT EDIT! File to edit: 05_exportDev.ipynb (unless otherwise specified).

__all__ = ['componentTemplate', 'chunkTemplate', 'rootStructTemplate', 'outLeftRight', 'recordLinks', 'checkForBreak',
           'nodeStat', 'finaliseComponentBase', 'processAnnotationInterval', 'updateEdges', 'baseLayerZoom',
           ...
           'searchIndicesGeneRecord', 'exportLayer', 'compLinksToAccCompLinks', 'recordZoomLevelForDebug',
           'exportToPantograph']
...
# Cell
def convertRemovableComponents(translateDict,linkLengths,pairedLinks,blockEdges,forwardLinks,isZoom=True):
    '''
    ...
    for blockLength,linkList in linkLengths.items():
        for linkFrom,linkTo in linkList:
            linkLengthsConv.setdefault(blockLength,set()).update(convertLink(linkFrom,linkTo,translateDict,forwardLinks,isZoom))
...
#             pdb.set_trace()
    ...
    linkLengthsConv,pairedLinksConv,blockEdgesConv = \
    convertRemovableComponents(nodeToComponent,linkLengths,pairedLinks,blockEdges,graph.forwardLinks,isZoom=False)

    return fromComponentLinks,toComponentLinks,linkLengthsConv,pairedLinksConv,blockEdgesConv
...
def convertBlocksToLengths(linksBlocks,nodeLengths):
    ...
    for link,blockList in linksBlocks.items():
        ...
        linkLengths.setdefault(max(blockListToLengths(blockList,nodeLengths)),set()).add(link)
...
# Cell
def recordLinkBlockAssociation(link,blockList,interconnectedLinks,linksBlocks):
...
# Cell
def processPseudoPair(breakPos,returnPos,pathID,pathNodeArray,combinedNodeDirArray,pathNextNode,nodeLengths,usedSecondInPairPath,pairedLinks,interconnectedLinks,linksBlocks):
    ...
    if returnLinkStart is not None:
        ...
        if not checkIndividualLink(lastNonLinearLink,pathID,usedSecondInPairPath):
            ...
            recordLinkBlockAssociation(lastNonLinearLink,blockList,interconnectedLinks,linksBlocks)
...
def processPathBreaks(pathBreakCoordPairs,
                       pathNodeArray,pathNextNode,combinedNodeDirArray,pathNodeInversionRate,pathLengths,nodeLengths):
    linksBlocks = {}
    pairedLinks = {}
    interconnectedLinks = {}
    linkStarts = {}
    ...
    for breakID,(pathID,breakPos) in enumerate(pathBreakCoordPairs):
        print(f'\rProcessing path break number {breakID}/{numPathBreaks}',end='')
        ...
        if len(nextPos)>0:
            ...
            if leftLink!=substituteLink and rightLink!=substituteLink:
                ...
                # follow-through links through empty components can be established. Implemented, but need testing.
                #################################################################################################
                ...
                recordLinkBlockAssociation(rightLink,blockList,interconnectedLinks,linksBlocks)
            else:
                ...
                blockList = processPseudoPair(breakPos,returnPos,pathID,pathNodeArray,combinedNodeDirArray,pathNextNode,nodeLengths,usedSecondInPairPath,pairedLinks,interconnectedLinks,linksBlocks)
            ...
            if len(loopPos)>0:
                ...
                returnPos = loopPos[0]
                blockList = processPseudoPair(breakPos,returnPos+1,pathID,pathNodeArray,combinedNodeDirArray,pathNextNode,nodeLengths,usedSecondInPairPath,pairedLinks,interconnectedLinks,linksBlocks)
            else:
                ...
                if curLinkFromDir!=curLinkToDir:
                    if curLinkFromDir>0:
                        ...
                        nextPosFrom = np.where(combinedNodeDirArray[pathID,:breakPos+1]==potentialNextFrom)[0][::-1]
                        nextPosTo = np.where(combinedNodeDirArray[pathID,:breakPos+2]==potentialNextTo)[0][::-1]
        ...
        processStartsEnds(leftLink,linkStarts,linkEnds,interconnectedLinks)
        if len(blockList)>0:

#             print(f"For path {pathID} and link {leftLink} block is {blockList}")
            recordLinkBlockAssociation(leftLink,blockList,interconnectedLinks,linksBlocks)

    postprocessLinksBlocks(linksBlocks,interconnectedLinks)

    print('\nProcessing path breaks finished.')
    return linksBlocks,pairedLinks
...
# Cell
def identifyRearrangementBlocks(nodesStructure):
    '''
...
def getRemovableStructures(graph = None,
    ...
    if nodeLengths is None or pathLengths is None or pathNodeArray is None or pathDirArray is None and pathNextNode is not None:
    ...
    linksBlocks,pairedLinks = processPathBreaks(pathBreakCoordPairs,
                      pathNodeArray,pathNextNode,combinedNodeDirArray,pathNodeInversionRate,pathLengths,nodeLengths)

    linkLengths = convertBlocksToLengths(linksBlocks,nodeLengths)

    blockEdges = identifyRearrangementBlocks(nodesStructure)

    return linkLengths,pairedLinks,blockEdges,pathNodeInversionRate
...
def updateLinks(newToOldInd,oldToNewInd,
                fromComponentLinks,toComponentLinks,
                linkLengths,pairedLinks,blockEdges,accStarts,accEnds,components,compAccDir,
                newFromComponentLinks={},newToComponentLinks={}):
    '''
...
#             fromCompDict = toComponentLinks.get(toComp,{}).get('-',{})
    ...
    linkLengthsConv,pairedLinksConv,blockEdgesConv = \
        convertRemovableComponents(oldToNewInd,linkLengths,pairedLinks,blockEdges,fromComponentLinks)

    return newFromComponentLinks,newToComponentLinks,accStarts,accEnds,linkLengthsConv,pairedLinksConv,blockEdgesConv
...
#                     binBlockLengthAddition = max([binBlockLengthAddition,binColLength])
    ...
    newFromComponentLinks,newToComponentLinks,accStarts,accEnds,newLinkLengths,newPairedLinks,newBlockEdges = \
    ...
    return newComponents,newComponentLengths,\
            newFromComponentLinks,newToComponentLinks,\
            accStarts,accEnds,newLinkLengths,newPairedLinks,newBlockEdges
#,newToOldInd,oldToNewInd
                ...
                if processPair:
                    ...
                    for toStrand,accessionList in list(toCompDict.items()):
                        if toStrand==fromStrand and link[0]+(1 if toStrand=='+' else -1)==link[1]:
                            fromComponentLinks[link[0]][fromStrand].setdefault(link[1],{}).setdefault(toStrand,[]).update(accessionLinks)
...
# Cell
def processCollapsibleBlocks(zoomLevel,linkLengths,pairedLinks,fromComponentLinks,toComponentLinks):
...
def updateLinksRemoveComp(oldToNewInd,fromComponentLinks,toComponentLinks,
                          linkLengths,pairedLinks,blockEdges,accStarts,accEnds):
    ...
    for blockLength,linkList in linkLengths.items():
        for link in linkList:
            if (link[0] not in oldToNewInd and link[1] not in oldToNewInd):
                continue
            elif (link[0] not in oldToNewInd or link[1] not in oldToNewInd):
            ...
            newLinkFrom = oldToNewInd[link[0]]
            newLinkTo = oldToNewInd[link[1]]
    ...
    for mainLink,pairedDict in pairedLinks.items():
        for pairedLink,subLink in pairedDict.items():
            isInIsolatedBlock = [side not in oldToNewInd for side in itertools.chain(mainLink,pairedLink,subLink)]
            if all(isInIsolatedBlock):
                continue
            elif all(isInIsolatedBlock):
            ...
            newMainLink = (oldToNewInd[mainLink[0]],oldToNewInd[mainLink[1]])
            newPairedLink = (oldToNewInd[pairedLink[0]],oldToNewInd[pairedLink[1]])
            newSubLink = (oldToNewInd[subLink[0]],oldToNewInd[subLink[1]])
            newPairedLinks.setdefault(mainLink,{})[pairedLink] = subLink

    # Update blockEdges
    newBlockEdges = {}
    for comp,blockLength in blockEdges.items():
        if comp in oldToNewInd:
            newBlockEdges[oldToNewInd[comp]] = blockLength

    return newFromComponentLinks,newToComponentLinks,accStarts,accEnds,newLinkLengths,newPairedLinks,newBlockEdges

# Cell
def removeIsolatedBlocks(isolatedBlockList,components,componentLengths,
                         fromComponentLinks,toComponentLinks,
                         accStarts,accEnds,linkLengths,pairedLinks,blockEdges):
    if len(isolatedBlockList)==0:
        return components,componentLengths,fromComponentLinks,toComponentLinks,accStarts,accEnds,linkLengths,pairedLinks,blockEdges
...
#     newToOldInd = {ind:[el] for ind,el in enumerate(newToOldInd)}
    ...
    # updating links
    fromComponentLinks,toComponentLinks,accStarts,accEnds,linkLengths,pairedLinks,blockEdges = \
...
# Cell
def clearInvisible(zoomLevel,linkLengths,pairedLinks,blockEdges,
                   fromComponentLinks,toComponentLinks,
                   accStarts,accEnds,
    ...
    # Link removal
    processCollapsibleBlocks(zoomLevel,linkLengths,pairedLinks,fromComponentLinks,toComponentLinks)
    ...
    components,componentLengths,fromComponentLinks,toComponentLinks,accStarts,accEnds,linkLengths,pairedLinks,blockEdges = \
...
#     toLinks = {}
    ...
    linkLengths,pairedLinks,blockEdges,pathNodeInversion = \
                getRemovableStructures(nodeLengths = nodeLengths, pathLengths = pathLengths,
                                       pathNodeArray = pathNodeArray, pathDirArray = pathDirArray,
                                       inversionThreshold = inversionThreshold)
    ...
    nodeToComponentLinks(components,componentToNode,nodeToComponent,
                         fromLinks,toLinks,graph,
                         fromComponentLinks,toComponentLinks,
                         linkLengths,pairedLinks,blockEdges,debug=debug)
...
#         print(pairedLinks)
        ...
        components,componentLengths,fromComponentLinks,toComponentLinks,accStarts,accEnds,linkLengths,pairedLinks,blockEdges = \
        clearInvisible(zoomLevel,linkLengths,pairedLinks,blockEdges,
                       fromComponentLinks,toComponentLinks,
        ...
        nextLayerZoom(zoomLevel,
                      ...
                      fromComponentLinks,toComponentLinks,graph,
                      accStarts,accEnds,
                      maxLengthComponent,linkLengths,pairedLinks,blockEdges,
                      inversionThreshold=inversionThreshold)
    ...
    if returnDebugData:
        return zoomComponentLengths,zoomNodeToComponent,zoomComponentToNodes,zoomComponents,zoomCompNucleotides,zoomAccStarts,zoomAccEnds, \
                invertedStarts,invertedEnds,toComponentLinks,fromComponentLinks,fromLinks,toLinks,graph,rootStruct